...
class TestManufacturingOrderAutoValidate(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.product_template = cls.env.ref("mrp.product_product_computer_desk_head_product_template")
    ...
    def _replenish_product(cls, product, product_qty=1, product_uom=None):
        ...
        wiz = cls.env["product.replenish"].with_context(default_product_id=product.id).create(
            {
    ...
    def test_keep_qty_on_replenishment(self):
        ...
        created_mos = self.env["mrp.production"].search([("id", "not in", existing_mos.ids)])
        ...
        self.assertFalse(any("raised" in m.body for m in created_mos.message_ids))
    ...
    def test_raise_qty_on_replenishment(self):
        ...
        self.assertEqual(len(created_mos), 1)
        self.assertEqual(created_mos.product_qty, self.bom.product_qty)
        self.assertTrue(any("raised" in m.body for m in created_mos.message_ids))
